...
class Translations extends Table {
  ...
  /// Полный текст карточки — сжатый (deflate), распаковывается при чтении.
  ///
  /// Описания составляли больше половины базы: 6 514 текстов, 16,5 МБ.
  /// Сжатие срезает девять мегабайт с приложения, а разворачивается один
  /// текст за доли миллисекунды — это дешевле, чем кажется, и происходит
  /// только при открытии карточки.
  ///
  /// Краткое `summary` намеренно осталось обычным текстом: по нему
  /// строятся списки и поисковый индекс, и распаковывать его на каждую
  /// строку выдачи было бы дороже, чем хранить как есть.
  BlobColumn get description => blob().nullable()();
...
/// Снимки, которые человек сделал сам.
///
/// Живут в пользовательском файле рядом с избранным: это его собственные
/// фотографии, и обновление контента не имеет права их тронуть. Сами
/// файлы лежат в каталоге приложения, здесь только путь и привязка.
///
/// Зачем это есть. Приложение знает, где человек был и что смотрел;
/// добавив к этому его собственные снимки, оно может собрать «мою поездку
/// по Норвегии» — карту с точками и фотографиями, которой приятно
/// поделиться. Всё считается на телефоне, никуда не отправляется, и
/// человек сам решает, публиковать картинку или нет.
///
/// Общих альбомов и чужих фотографий здесь не будет: это потребовало бы
/// сервера, модерации и ответственности за чужой контент. Добавить
/// социальную часть поверх личного дневника можно будет всегда; обратно
/// — уже нет.
@DataClassName('TripPhoto')
class TripPhotos extends Table {
  IntColumn get id => integer().autoIncrement()();

  /// К какому месту привязан снимок. Может быть пустым: человек
  /// фотографирует и то, что в каталоге не значится — дорогу, обед,
  /// оленя у обочины.
  TextColumn get placeId => text().nullable()();

  /// Путь к файлу в каталоге приложения.
  TextColumn get path => text()();

  /// Когда снято. Заполняется из метаданных снимка, если они есть,
  /// иначе временем добавления.
  IntColumn get takenAt => integer()();

  /// Где снято. Координаты берутся из GPS телефона в момент добавления,
  /// а если его нет — от привязанного места. Могут отсутствовать вовсе:
  /// снимок, выбранный из галереи без геометки, всё равно имеет право
  /// попасть в поездку.
  RealColumn get lat => real().nullable()();
  RealColumn get lon => real().nullable()();

  /// Подпись, если человек её написал.
