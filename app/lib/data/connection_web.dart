...
Future<QueryExecutor> openConnection() async {
  ...
  // В браузере файлов два быть не может: ATTACH к отдельному файлу здесь
  // недоступен, хранилище — OPFS или IndexedDB одним блоком. Поэтому
  // пользовательская таблица создаётся прямо в этой базе.
  //
  // Расхождение с устройством осознанное и безопасное: на телефоне
  // разделение нужно, чтобы обновление контента не стирало избранное,
  // а в браузере контент и так не обновляется пакетами — web остаётся
  // средой разработки.
  try {
    await result.resolvedExecutor.runCustom(
    ...
    // Таблица уже есть — обычное дело при повторном запуске.
  }
