...
**Этап 1 — карта (2 недели, Flutter)**
Flutter + MapLibre + локальный PMTiles. Одна цель: карта Норвегии рендерится в самолётном режиме.

**Этап 2 — контент (3–4 недели)**
Экраны городов и мест, SQLite через drift, FTS-поиск, галереи, избранное.

**Этап 3 — пакеты (2 недели)**
Менеджер загрузок, manifest, версионирование, докачка.

**Этап 4 — маршруты и полировка (3 недели)**
Готовые маршруты, GPX-экспорт, UI на всех шести языках (§8), тёмная тема.

**Этап 5 — релиз**
App Store + Google Play. Модель: базовый пакет бесплатно, региональные — разовая покупка или единая Pro-версия.
...
- [ ] Замерить размер PMTiles z0–14 по факту, а не по прикидке
