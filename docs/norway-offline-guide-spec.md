...
> **Как читать этот документ.** Это исходная спецификация; часть решений
> с тех пор изменилась по результатам замеров. Все расхождения перечислены
> в [[decisions]] — сверяйтесь с ним. Текущее состояние работ: [[status]].
> Карта документации: [[README]].
...
8. **Экстренная помощь** — телефоны служб спасения Норвегии, координаты
   пользователя для диктовки диспетчеру, неброская кнопка в шапке. Добавлено
   2026-09-10, см. [[decisions#Раздел экстренной помощи]]. Работает офлайн
   и без SIM-карты, то есть именно там, где всё остальное бесполезно.
...
**Обновлено 2026-09-09 по фактическому замеру.** Прежняя версия раздела была основана на
предположениях; ниже — числа из `cmd/probe` по 30 городам и 30 достопримечательностям.
Полный отчёт: `docs/probe-result.md`.

#### Города (30 объектов)

| Источник | Есть статья | Из них ≥1500 симв. | Средняя длина |
|---|---:|---:|---:|
| Wikipedia `en` | 100% | 90% | 19 777 |
| Wikipedia `no` | 100% | **100%** | **28 916** |
| Wikipedia `de` | 100% | **100%** | 12 869 |
| Wikivoyage `en` | 100% | **100%** | 24 263 |
| Wikivoyage `no` | раздела не существует | — | — |
| Wikivoyage `de` | 90% | 83% | 7 912 |

#### Достопримечательности (30 объектов)

| Источник | Есть статья | Из них ≥1500 симв. | Средняя длина |
|---|---:|---:|---:|
| Wikipedia `en` | 90% | 83% | 6 209 |
| Wikipedia `no` | **96%** | 86% | 9 852 |
| Wikipedia `de` | 76% | 60% | 4 497 |
| Wikivoyage `en` | 40% | 40% | 15 030 |
| Wikivoyage `no` | раздела не существует | — | — |
| Wikivoyage `de` | 16% | 16% | 7 696 |

#### Что из этого следует

**Основной источник — Wikipedia, а не Wikivoyage.** Исходная архитектура §3 строила
обогащение вокруг Wikivoyage. Для достопримечательностей это неверно: 40% против 90%
у Wikipedia. Wikivoyage силён только по городам, где у него формат путеводителя
(100%, в среднем 24 тыс. символов). Значит `cmd/enrich` двухисточниковый:
**города — Wikivoyage, места — Wikipedia**, с разными парсерами, потому что структура
статей у них разная.

**Норвежского Wikivoyage не существует вовсе** — прежняя формулировка «слабый»
неверна фактически, `no.wikivoyage.org` отдаёт HTML-заглушку.

**Норвежский — лучший источник из всех, а не проблемный.** Wikipedia `no` покрывает
100% городов и 96% мест, и статьи там длиннее английских. Для двух третей рынка
(внутренний туризм) контент бесплатен и качественен.

**Немецкий дёшев для городов и наполовину дёшев для мест.** Гипотеза «переводить почти
не нужно» подтверждается по городам (100% / 100%) и не подтверждается по местам
(76% статей, из них только 60% достаточной длины). По местам понадобится MT примерно
для трети объектов.

| Язык | План |
|---|---|
| `en` | Wikipedia для мест, Wikivoyage для городов — напрямую |
| `no` | Wikipedia, лучший источник; Wikivoyage отсутствует |
| `de` | напрямую по городам; по местам ~⅓ через MT |
| `es` | MT с ручной вычиткой топ-500 |
| `ru` | MT с вычиткой топ-500 |
| `zh` | только MT, помечать `quality < 50` |

**Ограничение замера:** проверены 60 заведомо известных объектов, то есть измерен
потолок, а не среднее. У безымянного водопада в Нурланне статьи не будет никогда.
Реальную долю покрытия по 15 000 POI даст подсчёт тега `wikidata` на первом прогоне
`cmd/extract` — это отдельный риск, см. §12.
...
- [x] Проверить реальный объём и качество текстов Wikivoyage по норвежским городам —
      **проверено 2026-09-09: 100% городов, в среднем 24 тыс. символов.** По местам
      Wikivoyage слаб (40%), там источник — Wikipedia. См. §8.5
- [x] Замерить фактическое покрытие `de.wikivoyage` — **проверено: 90% городов, но лишь
      16% достопримечательностей.** Немецкий бесплатен по городам, по местам нужен MT
      примерно для трети объектов. См. §8.5
