// Команда toplist дописывает в набор мест те объекты топ-20, которых нет
// в извлечении из OSM.
//
...
func main() {
	...
	for _, a := range toplist.All {
		...
		p := place{
			...
			Lat:      a.Lat,
			Lon:      a.Lon,
			// Топ-20 по определению значимее всего остального: список
	...
	fmt.Printf("\nТоп-20: уже в OSM %d, добавлено %d\n", existed, added)
	fmt.Printf("Файл: %s\n", *outPath)
}
