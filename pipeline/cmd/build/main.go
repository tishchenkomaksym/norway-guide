...
// nearestCity привязывает место к ближайшему городу в пределах радиуса.
//
// Привязка нужна для экрана города: «что посмотреть в Бергене». Радиус
// намеренно щедрый — норвежские достопримечательности часто лежат далеко
// за административной границей, но воспринимаются как часть города.
func nearestCity(lat, lon float64, cities []city, radiusKm float64) string {
	best := ""
	bestDist := radiusKm * 1000
